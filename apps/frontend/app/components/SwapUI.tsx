...
export function SwapUI({ market, price }: { market: string; price: string }) {
    ...
    const fetchBalance = async () => {
      if (activeTab === 'buy') {
        const balance = await axios.get(
          `${BASE_URL}/api/v1/getBalance?asset=${baseAsset}`,
          {
            headers: {
  ...
  const onPriceChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // const result = parseFloat(e.target.value) / parseFloat(price);
    // const formattedResult = result.toFixed(2);
    // setOrderQuantity(formattedResult);
  };
  ...
  const onButtonClick = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    if (!response) alert(`Error in post request while ${activeTab}ing`);
    // else if (response.status === 200) alert(`Order placed successfully`);
    else if (response.status !== 200) alert(`Received invalid response code`);
  };
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center justify-between flex-row">
                  ...
                  <p className="font-medium text-xs text-baseTextHighEmphasis">
                    {availableBalance} {activeTab == 'buy' ? 'USD' : 'TATA'}
                  </p>
                </div>
              </div>
            ...
            <button
              ...
              className={`font-semibold focus:ring-blue-200 focus:none focus:outline-none text-center h-12 rounded-xl text-base px-4 py-2 my-4 ${
                activeTab === 'buy'
                  ...
                  : 'bg-red-500 text-white active:scale-98'
              }`}
              data-rac=""
