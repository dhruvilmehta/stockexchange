...
//             <MarketHeader />
//             {tickers?.map((m) => <MarketRow key={Math.random()} market={m} />)}
//           </table>
//         </div>
//       </div>
//     </div>
...
//       <td className="px-1 py-3">
//         <p
//           className={`text-sm font-medium tabular-nums leading-5 text-sm text-greenText ${Number(market?.priceChange) > 0 ? 'text-green-500' : 'text-red-500'}`}
//         ></p>
//         {Number(market?.priceChange) > 0 ? '+' : ''}{' '}
//         {Number(market?.priceChange).toFixed(2)}{' '}
